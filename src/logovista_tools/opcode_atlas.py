...
CONTROL_SPECS: dict[int, ControlSpec] = {
    ...
    0x1A: ControlSpec(
        ...
        2,
        "layout",
        "tab/column position control",
        semantic_confidence="medium",
        notes=(
            "Observed in table-like runs such as element and era tables; "
            "payload values act as column stops/positions rather than visible text."
        ),
    ),
    0x1C: ControlSpec(
        "1c",
        2,
        "layout-media",
        "media block layout control",
        semantic_confidence="medium",
        notes=(
            "Observed as payload 2000 before 1f4d media references in IPHYCHE5; "
    ...
    0x62: ControlSpec("62", 6, "link", "body/cross-reference link end with pointer payload", pair_role="end", pair="42"),
